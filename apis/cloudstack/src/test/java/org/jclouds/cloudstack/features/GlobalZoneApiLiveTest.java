...
public class GlobalZoneApiLiveTest extends BaseCloudStackApiLiveTest {
	...
	public void testCreateUpdateDeleteZone() {
		...
		try {
			...
			assertNotNull(zone);
			assertEquals(zone,
					globalAdminClient.getZoneApi().getZone(zone.getId()));
			...
			Zone updated = globalAdminClient.getZoneApi().updateZone(
					zone.getId(),
					name(zoneName + "-2").externalDns(
							ImmutableList.of("8.8.4.4")));
	...
	public void testCreateZone() {
		skipIfNotGlobalAdmin();
		Zone zone = globalAdminClient.getZoneApi().createZone("jclouds-bz",
				NetworkType.BASIC, "8.8.8.8", "192.168.235.1");
	}
