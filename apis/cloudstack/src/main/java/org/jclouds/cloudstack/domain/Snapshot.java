...
public class Snapshot {
    ...
    */
	...
	public abstract static class Builder<T extends Builder<T>> {
		...
		protected Set<ResourceTag> tags;
		...
		public T revertable(boolean revertable) {
		...
		public Snapshot build() {
			return new Snapshot(id, account, created, domain, domainId,
					interval, jobId, jobStatus, name, snapshotType, state,
					volumeId, volumeName, volumeType, revertable, project,
					projectId, tags);
		}
	...
	@ConstructorProperties({ "id", "account", "created", "domain", "domainid",
			"intervaltype", "jobid", "jobstatus", "name", "snapshottype",
			"state", "volumeid", "volumename", "volumetype", "revertable",
			"project", "projectid", "tags" })
	protected Snapshot(String id, @Nullable String account,
			@Nullable Date created, @Nullable String domain,
			@Nullable String domainId, @Nullable Snapshot.Interval interval,
			@Nullable String jobId, @Nullable String jobStatus,
			@Nullable String name, @Nullable Snapshot.Type snapshotType,
			@Nullable Snapshot.State state, @Nullable String volumeId,
			@Nullable String volumeName, @Nullable Volume.Type volumeType,
			boolean revertable, @Nullable String project,
			@Nullable String projectId, @Nullable Set<ResourceTag> tags) {
	 ...
	 *         if the snapshot listed is part of a currently running
	 *         asynchronous job.
	 */
	@Nullable
	 ...
	 *         displayed if the snapshot listed is part of a currently running
	 *         asynchronous job.
	 */
	@Nullable
	...
	/**
	 * @return indicates whether the underlying storage supports reverting the
	 *         volume to this snapshot
	 */
	...
	/**
	 * @return the list of resource tags associated with snapshot
	 */
	...
	public boolean equals(Object obj) {
		...
		return Objects.equal(this.id, that.id)
				...
				&& Objects.equal(this.revertable, that.revertable)
	...
	protected ToStringHelper string() {
		return Objects.toStringHelper(this).add("id", id)
				...
				.add("interval", interval).add("jobId", jobId)
				.add("jobStatus", jobStatus).add("name", name)
				.add("snapshotType", snapshotType).add("state", state)
				.add("volumeId", volumeId).add("volumeName", volumeName)
				.add("volumeType", volumeType).add("revertable", revertable)
