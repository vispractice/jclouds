...
public class VirtualMachine {
    ...
    */
	...
	public abstract static class Builder<T extends Builder<T>> {
		...
		protected long diskKbsRead;
		protected long diskKbsWrite;
		...
		public T securityGroups(Set<SecurityGroup> securityGroups) {
			this.securityGroups = ImmutableSet.copyOf(checkNotNull(
		...
		public T diskIoRead(long diskIoRead) {
			this.diskIoRead = diskIoRead;
			return self();
		}

		public T diskIoWrite(long diskIoWrite) {
			this.diskIoWrite = diskIoWrite;
			return self();
		}

		public T diskKbsRead(long diskKbsRead) {
			this.diskKbsRead = diskKbsRead;
			return self();
		}

		public T getDiskKbsWrite(long diskKbsWrite) {
			this.diskKbsWrite = diskKbsWrite;
			return self();
		}

		public T isUsesVirtualNetwork(boolean usesVirtualNetwork) {
			this.usesVirtualNetwork = usesVirtualNetwork;
			return self();
		}
		...
		public T dynamicallyScalable(boolean dynamicallyScalable) {
		...
		public T getProjectId(String projectId) {
			this.projectId = projectId;
			return self();
		}

		public T getServiceState(String serviceState) {
		...
		public VirtualMachine build() {
			return new VirtualMachine(id, account, cpuCount, cpuSpeed, cpuUsed,
					displayName, created, domain, domainId, usesVirtualNetwork,
					group, groupId, guestOSId, HAEnabled, hostId, hostname,
					IPAddress, ISODisplayText, ISOId, ISOName, jobId,
					jobStatus, memory, name, networkKbsRead, networkKbsWrite,
		...
		public T fromVirtualMachine(VirtualMachine in) {
			return this.id(in.getId()).account(in.getAccount())
					.cpuCount(in.getCpuCount()).cpuSpeed(in.getCpuSpeed())
					.cpuUsed(in.getCpuUsedAsString())
					.displayName(in.getDisplayName()).created(in.getCreated())
					.domain(in.getDomain()).domainId(in.getDomainId())
					.usesVirtualNetwork(in.usesVirtualNetwork())
					.group(in.getGroup()).groupId(in.getGroupId())
					.guestOSId(in.getGuestOSId()).isHAEnabled(in.isHAEnabled())
					.hostId(in.getHostId()).hostname(in.getHostname())
					.IPAddress(in.getIPAddress())
					.ISODisplayText(in.getISODisplayText())
					.ISOId(in.getISOId()).ISOName(in.getISOName())
					.jobId(in.getJobId()).jobStatus(in.getJobStatus())
					.memory(in.getMemory()).name(in.getName())
					.networkKbsRead(in.getNetworkKbsRead())
					.networkKbsWrite(in.getNetworkKbsWrite())
					.password(in.getPassword())
					.passwordEnabled(in.isPasswordEnabled())
					.publicIP(in.getPublicIP()).publicIPId(in.getPublicIPId())
	...
	private final long diskIoRead;
	private final long diskIoWrite;
	private final long diskKbsRead;
	private final long diskKbsWrite;
	...
	private final boolean dynamicallyScalable;
	...
	@ConstructorProperties({ "id", "account", "cpunumber", "cpuspeed",
			"cpuused", "displayname", "created", "domain", "domainid",
			"forvirtualnetwork", "group", "groupid", "guestosid", "haenable",
			"hostid", "hostname", "ipaddress", "isodisplaytext", "isoid",
			"isoname", "jobid", "jobstatus", "memory", "name",
			"networkkbsread", "networkkbswrite", "password", "passwordenabled",
	...
	protected VirtualMachine(String id, @Nullable String account,
			...
			boolean usesVirtualNetwork, @Nullable String group,
			@Nullable String groupId, @Nullable String guestOSId,
			boolean HAEnabled, @Nullable String hostId,
			@Nullable String hostname, String IPAddress, String ISODisplayText,
			@Nullable String ISOId, @Nullable String ISOName,
			@Nullable String jobId, @Nullable Integer jobStatus, long memory,
			@Nullable String name, @Nullable Long networkKbsRead,
			@Nullable Long networkKbsWrite, @Nullable String password,
			boolean passwordEnabled, @Nullable String publicIP,
			@Nullable String publicIPId, @Nullable String rootDeviceId,
			@Nullable String rootDeviceType,
			@Nullable String serviceOfferingId,
			@Nullable String serviceOfferingName,
			@Nullable VirtualMachine.State state,
			...
			boolean dynamicallyScalable,
			@Nullable String keypair,
			@Nullable String project,
			@Nullable String projectId,
			@Nullable String serviceState) {
		Preconditions.checkArgument(
				Strings.isNullOrEmpty(cpuUsed)
						|| cpuUsed.matches("^[0-9\\.|,\\-]+%$"),
				"cpuUsed value should be a decimal number followed by %");
	...
	public float getCpuUsed() {
		return cpuUsed != null ? Float.parseFloat(cpuUsed.substring(0,
				cpuUsed.length() - 1).replace(',', '.')) : 0.0f;
	}
	...
	/**
	 * @return Template details in key/value pairs
	 */
	...
	/**
	 * @return the read (io) of disk on the vm
	 */
	...
	/**
	 * @return the write (io) of disk on the vm
	 */
	...
	/**
	 * @return the read (bytes) of disk on the vm
	 */
	...
	/**
	 * @return the write (bytes) of disk on the vm
	 */
	...
	/**
	 * @return instance name of the user vm; this parameter is returned to the ROOT admin only
	 */
	...
	/**
	 * @return true if vm contains XS/VMWare tools inorder to support dynamic scaling of VM cpu/memory
	 */
	...
	/**
	 * @return the project name of the vm
	 */
	...
	/**
	 * @return State of the Service from LB rule
	 */
	...
	public int hashCode() {
		return Objects.hashCode(id, account, cpuCount, cpuSpeed, cpuUsed,
				displayName, created, domain, domainId, usesVirtualNetwork,
				group, groupId, guestOSId, HAEnabled, hostId, hostname,
				IPAddress, ISODisplayText, ISOId, ISOName, jobId, jobStatus,
				memory, name, networkKbsRead, networkKbsWrite, password,
	...
	public boolean equals(Object obj) {
		...
		return Objects.equal(this.id, that.id)
				...
				&& Objects.equal(this.serviceOfferingName,
						that.serviceOfferingName)
				&& Objects.equal(this.state, that.state)
				&& Objects.equal(this.templateDisplayText,
						that.templateDisplayText)
				...
				&& Objects.equal(this.diskIoRead, that.diskIoRead)
				&& Objects.equal(this.diskIoWrite, that.diskIoWrite)
				&& Objects.equal(this.diskKbsRead, that.diskKbsRead)
				&& Objects.equal(this.diskKbsWrite, that.diskKbsWrite)
				&& Objects.equal(this.instanceName, that.instanceName)
				&& Objects.equal(this.dynamicallyScalable, that.dynamicallyScalable)
				&& Objects.equal(this.keypair, that.keypair)
	...
	protected ToStringHelper string() {
		return Objects.toStringHelper(this).add("id", id)
				.add("account", account).add("cpuCount", cpuCount)
				.add("cpuSpeed", cpuSpeed).add("cpuUsed", cpuUsed)
				.add("displayName", displayName).add("created", created)
				.add("domain", domain).add("domainId", domainId)
				.add("usesVirtualNetwork", usesVirtualNetwork)
				.add("group", group).add("groupId", groupId)
				.add("guestOSId", guestOSId).add("HAEnabled", HAEnabled)
				.add("hostId", hostId).add("hostname", hostname)
				.add("IPAddress", IPAddress)
				.add("ISODisplayText", ISODisplayText).add("ISOId", ISOId)
				.add("ISOName", ISOName).add("jobId", jobId)
				.add("jobStatus", jobStatus).add("memory", memory)
				...
				.add("publicIP", publicIP).add("publicIPId", publicIPId)
				.add("rootDeviceId", rootDeviceId)
				.add("rootDeviceType", rootDeviceType)
				.add("serviceOfferingId", serviceOfferingId)
				.add("serviceOfferingName", serviceOfferingName)
				.add("state", state)
				...
				.add("details", details)
				.add("diskIoRead", diskIoRead)
				.add("diskIoWrite", diskIoWrite)
				.add("diskKbsWrite", diskKbsWrite)
				.add("diskKbsRead", diskKbsRead)
				.add("instanceName", instanceName)
				.add("dynamicallyScalable", dynamicallyScalable)
				.add("keypair", keypair)
				.add("project", project)
