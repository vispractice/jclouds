...
public class Host implements Comparable<Host> {
	...
	public static enum ClusterType {
		CLOUD_MANAGED, EXTERNAL_MANAGED, UNKNOWN;
	...
	public static enum State {
		CONNECTING, UP, DOWN, DISCONNECTED, UPDATING, PREPARE_FOR_MAINTENANCE, ERROR_IN_MAINTENANCE, 
		MAINTENANCE, ALERT, REMOVED, REBALANCING, ENABLED, DISABLED, UNMANAGED, UNKNOWN;
	...
	public static enum Type {
		STORAGE, ROUTING, SECONDARY_STORAGE, SECONDARY_STORAGE_CMD_EXECUTOR, CONSOLE_PROXY, EXTERNAL_FIREWALL, EXTERNAL_LOAD_BALANCER, PXE_SERVER, TRAFFIC_MONITOR, EXTERNAL_DHCP, SECONDARY_STORAGE_VM, LOCAL_SECONDARY_STORAGE, UNKNOWN;
		...
		public String toString() {
			if (this == SECONDARY_STORAGE_VM) {
				return "SecondaryStorageVM"; // note the inconsistency in VM
												// naming
			}
			return UPPER_UNDERSCORE.to(UPPER_CAMEL, name());
		}
	...
	public abstract static class Builder<T extends Builder<T>> {
		...
		public T cpuSockets(int cpuSockets) {
			this.cpuSockets = cpuSockets;
			return self();
		}

		public T hahost(boolean hahost) {
			this.hahost = hahost;
			return self();
		}

		public T hypervisorVersion(String hypervisorVersion) {
			this.hypervisorVersion = hypervisorVersion;
			return self();
		}

		public T resourceState(String resourceState) {
			this.resourceState = resourceState;
			return self();
		}

		public T suitableForMigration(boolean suitableForMigration) {
		...
		public Host build() {
			return new Host(id, allocationState, averageLoad, capabilities,
					...
					events, hasEnoughCapacity, tags.build(), hypervisor,
					ipAddress, localStorageActive, jobId, jobStatus,
					lastPinged, managementServerId, memoryAllocated,
					memoryTotal, memoryUsed, name, networkKbsRead,
					networkKbsWrite, osCategoryId, osCategoryName, podId,
					podName, removed, state, type, version, zoneId, zoneName,
					cpuSockets, hahost, hypervisorVersion, resourceState, suitableForMigration);
		}

		public T fromHost(Host in) {
			return this.id(in.getId()).allocationState(in.getAllocationState())
					...
					.cpuNumber(in.getCpuNumber()).cpuSpeed(in.getCpuSpeed())
					...
					.memoryUsed(in.getMemoryUsed()).name(in.getName())
					...
					.podId(in.getPodId()).podName(in.getPodName())
					.removed(in.getRemoved()).state(in.getState())
					.type(in.getType()).version(in.getVersion())
					.zoneId(in.getZoneId()).zoneName(in.getZoneName())
					.cpuSockets(in.getCpuSockets()).hahost(in.isHahost())
					.hypervisorVersion(in.getHypervisorVersion()).resourceState(in.getResourceState())
					.suitableForMigration(in.isSuitableForMigration());
		}
	}
	...
	private final boolean hahost;
	private final String hypervisorVersion;
	private final String resourceState;
	private final boolean suitableForMigration;

	@ConstructorProperties({ "id", "allocationstate", "averageload",
			"capabilities", "clusterid", "clustername", "clustertype",
			"cpuallocated", "cpunumber", "cpuspeed", "cpuused",
			"cpuwithoverprovisioning", "created", "disconnected",
			"disksizeallocated", "disksizetotal", "events",
			"hasenoughcapacity", "hosttags", "hypervisor", "ipaddress",
			"islocalstorageactive", "jobid", "jobstatus", "lastpinged",
			"managementserverid", "memoryallocated", "memorytotal",
			...
			"cpusockets", "hahost", "hypervisorversion", "resourcestate", "suitableformigration"})
	protected Host(String id, @Nullable AllocationState allocationState,
			int averageLoad, @Nullable String capabilities,
			...
			@Nullable Iterable<String> tags, @Nullable String hypervisor,
			@Nullable String ipAddress, boolean localStorageActive,
			@Nullable String jobId, @Nullable AsyncJob.Status jobStatus,
			@Nullable Date lastPinged, @Nullable String managementServerId,
			long memoryAllocated, long memoryTotal, long memoryUsed,
			@Nullable String name, long networkKbsRead, long networkKbsWrite,
			@Nullable String osCategoryId, @Nullable String osCategoryName,
			@Nullable String podId, @Nullable String podName,
			@Nullable Date removed, @Nullable Host.State state,
			@Nullable Host.Type type, @Nullable String version,
			@Nullable String zoneId, @Nullable String zoneName,
			int cpuSockets, boolean hahost, @Nullable String hypervisorVersion, 
			@Nullable String resourceState, boolean suitableForMigration) {
	...
	/**
	 * @return the number of CPU sockets on the host
	 */
	...
	/**
	 * @return true if the host is Ha host (dedicated to vms started by HA process; false otherwise
	 */
	...
	/**
	 * @return the resource state of the host
	 */
	@Nullable
	...
	/**
	 * @return true if this host is suitable(has enough capacity and satisfies all conditions like hosttags, max guests vm limit etc) to migrate a VM to it , false otherwise
	 */
	...
	public int hashCode() {
		return Objects.hashCode(id, allocationState, averageLoad, capabilities,
				clusterId, clusterName, clusterType, cpuAllocated, cpuNumber,
				cpuSpeed, cpuUsed, cpuWithOverProvisioning, created,
				disconnected, diskSizeAllocated, diskSizeTotal, events,
				hasEnoughCapacity, tags, hypervisor, ipAddress,
				localStorageActive, jobId, jobStatus, lastPinged,
				managementServerId, memoryAllocated, memoryTotal, memoryUsed,
				name, networkKbsRead, networkKbsWrite, osCategoryId,
				osCategoryName, podId, podName, removed, state, type, version,
				zoneId, zoneName, cpuSockets, hahost, hypervisorVersion, 
				resourceState, suitableForMigration);
	}
	...
	public boolean equals(Object obj) {
		...
		return Objects.equal(this.id, that.id)
				...
				&& Objects.equal(this.managementServerId,
						that.managementServerId)
				...
				&& Objects.equal(this.cpuSockets, that.cpuSockets)
				&& Objects.equal(this.hahost, that.hahost)
				&& Objects.equal(this.hypervisorVersion, that.hypervisorVersion)
				&& Objects.equal(this.resourceState, that.resourceState)
				&& Objects.equal(this.suitableForMigration, that.suitableForMigration);
	}

	protected ToStringHelper string() {
		return Objects.toStringHelper(this).add("id", id)
				.add("allocationState", allocationState)
				.add("averageLoad", averageLoad)
				.add("capabilities", capabilities).add("clusterId", clusterId)
				.add("clusterName", clusterName)
				.add("clusterType", clusterType)
				.add("cpuAllocated", cpuAllocated).add("cpuNumber", cpuNumber)
				.add("cpuSpeed", cpuSpeed).add("cpuUsed", cpuUsed)
				.add("cpuWithOverProvisioning", cpuWithOverProvisioning)
				.add("created", created).add("disconnected", disconnected)
				.add("diskSizeAllocated", diskSizeAllocated)
				.add("diskSizeTotal", diskSizeTotal).add("events", events)
				.add("hasEnoughCapacity", hasEnoughCapacity).add("tags", tags)
				.add("hypervisor", hypervisor).add("ipAddress", ipAddress)
				.add("localStorageActive", localStorageActive)
				.add("jobId", jobId).add("jobStatus", jobStatus)
				.add("lastPinged", lastPinged)
				.add("managementServerId", managementServerId)
				.add("memoryAllocated", memoryAllocated)
				.add("memoryTotal", memoryTotal).add("memoryUsed", memoryUsed)
				.add("name", name).add("networkKbsRead", networkKbsRead)
				.add("networkKbsWrite", networkKbsWrite)
				.add("osCategoryId", osCategoryId)
				.add("osCategoryName", osCategoryName).add("podId", podId)
				.add("podName", podName).add("removed", removed)
				.add("state", state).add("type", type).add("version", version)
				.add("zoneId", zoneId).add("zoneName", zoneName)
				.add("cpuSockets", cpuSockets).add("hahost", hahost)
				.add("hypervisorVersion", hypervisorVersion).add("resourceState", resourceState)
				.add("suitableForMigration", suitableForMigration);
	}
