...
public class Template implements Comparable<Template> {
	public enum Status {
		...
		/**
		 * status of download is not known. Example - When the job for
		...
		ABANDONED,
		/**
		 * the download has reached an error state. Example - there is not route
		 * to ssvm agent
		 */
		DOWNLOAD_ERROR,
		/**
		...
		// CS(ironically shown
		// as download template in the UI, API - extractTemplate ). Some of the
		// generic states (like
	...
	public static enum Type {
		...
		// TODO do we need camel case routines (e.g. see enums in
		// VirtualMachine) ?
	...
	public abstract static class Builder<T extends Builder<T>> {
		...
		public T sshkeyEnabled(boolean sshkeyEnabled) {
		...
		public Template build() {
			return new Template(id, displayText, domain, domainId, account,
					accountId, zone, zoneId, OSType, OSTypeId, name, type,
		...
		public T fromTemplate(Template in) {
			return this.id(in.getId()).displayText(in.getDisplayText())
					.domain(in.getDomain()).domainId(in.getDomainId())
					.account(in.getAccount()).accountId(in.getAccountId())
					.zone(in.getZone()).zoneId(in.getZoneId())
					.OSType(in.getOSType()).OSTypeId(in.getOSTypeId())
					.name(in.getName()).type(in.getType())
					.status(in.getStatus()).format(in.getFormat())
					.hypervisor(in.getHypervisor()).size(in.getSize())
	...
	private final boolean sshkeyEnabled;
	...
	@ConstructorProperties({ "id", "displaytext", "domain", "domainid",
			...
			"ostypeid", "name", "templatetype", "status", "format",
			"hypervisor", "size", "created", "removed", "crossZones",
	...
	protected Template(String id, @Nullable String displayText,
			...
			@Nullable String zone, @Nullable String zoneId,
			@Nullable String OSType, @Nullable String OSTypeId,
			@Nullable String name, @Nullable Template.Type type,
			@Nullable Template.Status status, @Nullable Template.Format format,
	 ...
	 *
	 * <p>
	 * Note that in CloudStack 2.2.x through to at least 3.0.4, the possible
	 * status values are not well defined by CloudStack. CloudStack returns a
	 * plain-text English string for UI display, which jclouds attempts to parse
	 * into an enumeration, but the mapping is incomplete. This method should be
	 * reliable for the common cases, but it is possible (particularly for error
	...
	/**
	 * @return true if the template is ready to be deployed from, false
	 *         otherwise
	 */
	...
	protected ToStringHelper string() {
		return Objects.toStringHelper(this).add("id", id)
				.add("displayText", displayText).add("domain", domain)
				.add("domainId", domainId).add("account", account)
				.add("accountId", accountId).add("zone", zone)
				.add("zoneId", zoneId).add("OSType", OSType)
				.add("OSTypeId", OSTypeId).add("name", name).add("type", type)
				.add("status", status).add("format", format)
				.add("hypervisor", hypervisor).add("size", size)
