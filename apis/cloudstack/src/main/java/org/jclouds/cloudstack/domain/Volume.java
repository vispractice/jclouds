...
public class Volume {
    ...
    */
	public static enum State {
		...
		/**
		 * indicates that the volume record is created in the DB, but not on the
		 * backend
		 */
		ALLOCATED,
		/**
		...
		READY,
		/**
		 * the volume is destroyed (either as a result of deleteVolume command
		 * for DataDisk or as a part of destroyVm)
		 */
		DESTROYED,
		/**
		 * the volume has failed somehow, e.g. during creation (in cloudstack
		 * development)
		 */
		FAILED,
    ...
    */
	public static enum Type {
		ROOT(0), DATADISK(1), UNRECOGNIZED(Integer.MAX_VALUE);
	...
	public abstract static class Builder<T extends Builder<T>> {
		...
		protected String displayVolume;
		...
		public T displayVolume(String displayVolume) {
			this.displayVolume = displayVolume;
			return self();
		}

		public T maxIops(long maxIops) {
			this.maxIops = maxIops;
			return self();
		}

		public T minIops(long minIops) {
		...
		public T quiesceVm(String quiesceVm) {
		...
		public T storageId(String storageId) {
		...
		public Volume build() {
			return new Volume(id, account, attached, created, destroyed,
					deviceId, diskOfferingDisplayText, diskOfferingId,
					diskOfferingName, domain, domainId, hypervisor,
					isExtractable, jobId, jobStatus, name,
					serviceOfferingDisplayText, serviceOfferingId,
					serviceOfferingName, size, snapshotId, state, storage,
					storageType, type, virtualMachineId, vmDisplayName, vmName,
		...
		public T fromVolume(Volume in) {
			return this
					...
					.size(in.getSize()).snapshotId(in.getSnapshotId())
					.state(in.getState()).storage(in.getStorage())
					.storageType(in.getStorageType()).type(in.getType())
					.virtualMachineId(in.getVirtualMachineId())
					.vmDisplayName(in.getVmDisplayName())
					.vmName(in.getVmName()).vmState(in.getVmState())
					.zoneId(in.getZoneId()).zoneName(in.getZoneName())
					.diskBytesReadRate(in.getDiskBytesReadRate())
					.diskBytesWriteRate(in.getDiskBytesWriteRate())
					.diskIopsReadRate(in.getDiskIopsReadRate())
					.diskIopsWriteRate(in.getDiskIopsWriteRate())
					.displayVolume(in.getDisplayVolume())
	...
	private final long diskBytesReadRate;
	private final long diskBytesWriteRate;
	private final long diskIopsReadRate;
	private final long diskIopsWriteRate;
	...
	@ConstructorProperties({ "id", "account", "attached", "created",
			...
			"diskofferingid", "diskofferingname", "domain", "domainid",
			"hypervisor", "isextractable", "jobid", "jobstatus", "name",
			"serviceofferingdisplaytext", "serviceofferingid",
			"serviceofferingname", "size", "snapshotid", "state", "storage",
			"storagetype", "type", "virtualmachineid", "vmdisplayname",
	...
	protected Volume(String id, @Nullable String account,
			...
			@Nullable String diskOfferingDisplayText,
			@Nullable String diskOfferingId, @Nullable String diskOfferingName,
			@Nullable String domain, @Nullable String domainId,
			@Nullable String hypervisor, boolean isExtractable,
			@Nullable String jobId, @Nullable String jobStatus,
			@Nullable String name, @Nullable String serviceOfferingDisplayText,
			@Nullable String serviceOfferingId,
			@Nullable String serviceOfferingName, long size,
			@Nullable String snapshotId, @Nullable Volume.State state,
			@Nullable String storage, @Nullable String storageType,
			@Nullable Volume.Type type, @Nullable String virtualMachineId,
			@Nullable String vmDisplayName, @Nullable String vmName,
			@Nullable VirtualMachine.State vmState, @Nullable String zoneId,
			@Nullable String zoneName, long diskBytesReadRate,
			long diskBytesWriteRate, long diskIopsReadRate,
			long diskIopsWriteRate, @Nullable String displayVolume,
	...
	public int hashCode() {
		return Objects.hashCode(id, account, attached, created, destroyed,
				deviceId, diskOfferingDisplayText, diskOfferingId,
				diskOfferingName, domain, domainId, hypervisor, isExtractable,
				jobId, jobStatus, name, serviceOfferingDisplayText,
				serviceOfferingId, serviceOfferingName, size, snapshotId,
				state, storage, storageType, type, virtualMachineId,
	...
	public boolean equals(Object obj) {
		...
		return Objects.equal(this.id, that.id)
				...
				&& Objects.equal(this.diskOfferingDisplayText,
						that.diskOfferingDisplayText)
				...
				&& Objects.equal(this.serviceOfferingDisplayText,
						that.serviceOfferingDisplayText)
				&& Objects
				...
				&& Objects
						.equal(this.diskBytesReadRate, that.diskBytesReadRate)
				&& Objects.equal(this.diskBytesWriteRate,
						that.diskBytesWriteRate)
				&& Objects.equal(this.diskIopsReadRate, that.diskIopsReadRate)
				&& Objects
						.equal(this.diskIopsWriteRate, that.diskIopsWriteRate)
				&& Objects.equal(this.displayVolume, that.displayVolume)
				&& Objects.equal(this.maxIops, that.maxIops)
				&& Objects.equal(this.minIops, that.minIops)
	...
	protected ToStringHelper string() {
		return Objects.toStringHelper(this).add("id", id)
				.add("account", account).add("attached", attached)
				.add("created", created).add("destroyed", destroyed)
				.add("deviceId", deviceId)
				.add("diskOfferingDisplayText", diskOfferingDisplayText)
				.add("diskOfferingId", diskOfferingId)
				.add("diskOfferingName", diskOfferingName)
				.add("domain", domain).add("domainId", domainId)
				.add("hypervisor", hypervisor)
				.add("isExtractable", isExtractable).add("jobId", jobId)
				.add("jobStatus", jobStatus).add("name", name)
				.add("serviceOfferingDisplayText", serviceOfferingDisplayText)
				.add("serviceOfferingId", serviceOfferingId)
				.add("serviceOfferingName", serviceOfferingName)
				.add("size", size).add("snapshotId", snapshotId)
				.add("state", state).add("storage", storage)
				.add("storageType", storageType).add("type", type)
				.add("virtualMachineId", virtualMachineId)
				.add("vmDisplayName", vmDisplayName).add("vmName", vmName)
				.add("vmState", vmState).add("zoneId", zoneId)
				.add("zoneName", zoneName)
				.add("diskBytesReadRate", diskBytesReadRate)
				.add("diskBytesWriteRate", diskBytesWriteRate)
				.add("diskIopsReadRate", diskIopsReadRate)
				.add("diskIopsWriteRate", diskIopsWriteRate)
				.add("displayVolume", displayVolume).add("maxIops", maxIops)
